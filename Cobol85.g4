...
* numerous COBOL files from banking and insurance.
*
* --- Merged single-pass grammar --------------------------------------------------
* This grammar merges the former Cobol85Preprocessor.g4 into Cobol85.g4 so that
* a raw COBOL source file can be parsed into a complete AST in a single pass.
*
* The preprocessor constructs (compilerOptions / PROCESS, COPY, REPLACE,
* EXEC CICS / SQL / SQLIMS, EJECT / SKIP / TITLE) are recognized as AST nodes
* instead of being executed by a separate preprocessor pass. EXEC blocks are
* parsed inline through a free-form body (charData) rather than relying on
* pre-inserted marker tokens. A downstream tool (e.g. a Python driver) can walk
* the tree and perform COPY/REPLACE expansion if needed; the grammar itself does
* no file I/O or token rewriting.
*
...
* The merged-in rules therefore do not consume NEWLINE and let the surrounding
* context own the terminating DOT_FS.
*/
...
startRule
    : compilerOptions? compilationUnit EOF
    ;
...
// --- preprocessor constructs (merged from Cobol85Preprocessor.g4) ------------------------------
// These rules recognize COPY / REPLACE / compiler options / compiler directives
// as AST nodes. They do not execute preprocessing (no file I/O, no token
// rewriting). Rule cores intentionally do NOT consume DOT_FS; the surrounding
// context (sentence, the dataDirective wrapper, or startRule) owns the
// terminator. NEWLINE/WS stay on the HIDDEN channel, so they are not mentioned.

// - compiler options (PROCESS / CBL) ----------------------------------
// Recognized tolerantly: the option stream is captured as words, literals and
// punctuation rather than enumerating ~250 option-specific tokens. Enumerating
// them would force many short abbreviations (single letters such as C, E, S)
// into the lexer and steal them from IDENTIFIER. The option tokens therefore
// remain usable as ordinary COBOL words elsewhere.

compilerOptions
    : (PROCESS | CBL) compilerOptionItem+ DOT_FS?
    ;

compilerOptionItem
    // Any token except the ones that begin the IDENTIFICATION division, so the
    // greedy option run stops cleanly before the program. This lets option
    // values reuse reserved keywords (EXTEND, COMPAT, ...) without forcing them
    // into cobolWord, and avoids importing ~250 option-specific lexer tokens.
    : ~(IDENTIFICATION | ID)
    ;
...
copySource
    : (literal | cobolWord (DOT cobolWord)*) ((OF | IN) copyLibrary)?
    ;
...
pseudoText
    : DOUBLEEQUALCHAR pseudoTextChar* DOUBLEEQUALCHAR
    ;
...
// - exec cics / sql / sql ims free-form bodies (parsed inline, no marker tokens) ----
// The body of an EXEC block is an arbitrary token run terminated by END_EXEC.
// Hidden whitespace (NEWLINE/WS) is skipped by the parser automatically.
...
// COPY / REPLACE / compiler directives inside a data division section. The
// directive cores do not own DOT_FS, so it is consumed here, matching the
// dataDescriptionEntry convention where each entry terminates on its own DOT_FS.

dataDirective
    : (copyStatement | replaceArea | replaceOffStatement | ejectStatement | skipStatement | titleStatement) DOT_FS
    ;
...
dataDescriptionEntryExecSql
    : EXEC SQL execCharData END_EXEC DOT_FS?
    ;
...
execCicsStatement
    : EXEC CICS execCharData END_EXEC DOT_FS?
    ;
...
execSqlImsStatement
    : EXEC SQLIMS execCharData END_EXEC DOT_FS?
    ;
...
// comment entry
//
// In the original two-pass design the preprocessor marked comment-entry lines
// with a *>CE tag so the grammar could match COMMENTENTRYLINE+. With a single
// pass we instead bound the free text by excluding the tokens that start the
// next identification paragraph or a division. This is best-effort: a comment
// entry that happens to contain one of those exact words will delimit early.
...
commentEntryChar
    : ~(AUTHOR | INSTALLATION | DATE_WRITTEN | DATE_COMPILED | SECURITY | REMARKS | ENVIRONMENT | DATA | PROCEDURE)
    ;

// lexer rules --------------------------------------------------------------------------------

// keywords
// --- merged preprocessor keywords (EXEC blocks, compiler options, directives) ---
CBL
    : C B L
    ;
